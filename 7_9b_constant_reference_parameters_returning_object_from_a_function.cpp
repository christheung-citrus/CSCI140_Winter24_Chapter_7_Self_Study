...
class InventoryItem
	...
	public:
		...
		// const functions. This ensures they cannot alter any class member data
...
****
* createItem
* This function stores user input data in the
members of a locally defined InventoryItem object, then returns
it. *
...
* showValues
*
* This function displays the member data in the
InventoryItem object passed to it. Because it was passed as a
constant reference, showValues accesses the original object, not a
copy, but it can only call member functions declared to be
const. This prevents it from calling any mutator
functions. *
